					...
					function() {
						...
						if ($j(this).attr("name") == "weixin") {
							...
							if ($j(this).attr("name") == "mail" || $j(this).attr("name") == "QQ") {
								...
								window.open("http://s.jiathis.com/?webid=" + $j(this).attr("name") + "&url=" + encodeURIComponent(o.url) + "&title=" + encodeURIComponent(o.word) + "&uid=1626433&jid=1344496797938582&pic="+encodeURIComponent(o.pic) , "腾讯网ishare", "height=600, width=560,toolbar=no,menubar=no,scrollbars=no,resizable=yes,location=no,status=no,top=200,left=200");
							}
						}
						return false;
					});
